    ...
    protected function getStats(): array
    {
        return [
            Stat::make('Total Projects', Project::count())
                ->description('Published: '.Project::where('status', ProjectStatus::Completed)->count())
                ->descriptionIcon('heroicon-o-briefcase')
                ->chart([7, 3, 10, 5, 15, 8, 12])
                ->color('success'),
            Stat::make('Total Leads', Lead::count())
                ->description('New: '.Lead::where('status', LeadStatus::New)->count().' | Anonymous: '.Lead::anonymous()->count())
